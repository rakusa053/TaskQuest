...
最終更新: 2026-04-15（**ver.12 完成** / git tag: v12.0）
...
## ✅ ver.12 完成（2026-04-15 / git tag: v12.0）

ver.12 タイトル：**アプリ一覧表示修正・README追加**

ver.12 で追加・修正した機能：
- **ブロックアプリ一覧に全アプリ表示されない問題修正**: Android 11以降のパッケージ可視性制限に対応。`QUERY_ALL_PACKAGES` 権限と `<queries>` に `LAUNCHER` インテントを追加し、LINE・YouTube など全アプリが表示されるよう修正
- **README.md 追加**: アプリ概要・技術スタック・アーキテクチャ・セットアップ手順を記載。アプリブロック機能を最大の特徴として全面に掲載
...
## ✅ ver.8 完成（2026-04-09 / git tag: v8.0）

ver.8 タイトル：**コインを追加**

ver.8 で追加・修正した機能：
- **ガチャハズレ追加**: ガチャにハズレ（確率25%）を追加。ハズレ時はコイン50枚を即時付与し、Firestore には保存しない
- **ハズレカード表示修正**: GachaCard の miss レアリティ専用色（灰色系）を適用。コイン獲得数と「コイン獲得」テキストを表示
- **再ガチャ時カードリセット修正**: `key={result.id}` を追加し、スピンごとにカードコンポーネントをリマウントして表示がリセットされるよう修正
- **チケット数即時更新**: スピン時に `profileStore` のチケット数を楽観的にデクリメント、API完了後にサーバー値で同期
- **テスト用10秒解放**: 報酬使用画面に制限なしで使える10秒解放ボタンを追加
- **プロフィールに解放タイマー表示**: アプリ解放中の残り時間をプロフィール画面上部に表示
- **報酬使用画面へのナビゲーション**: プロフィール画面から「報酬を使う（アプリ解放）」ボタンで直接遷移
